...
| Scatter Boost Spin | Player activates `scatter_boost`; frontend mode key currently `SCATTER_BOOST` | Configured cost `3.0x` | Same active reel strips as base (`BR0` for basegame, `FR0` for freegame); direct freegame distribution quota is `0.108` versus Base `0.100`; including the separate `0.001` wincap feature-entry path, total forced feature-entry allocation is `0.109` versus Base `0.101` | Can trigger Free Spins; can collect Legacy Keys; Legacy scatter credit can apply | Same result types as base, with higher free-game distribution quota | Remains in basegame after result unless Free Spins are triggered |
...
| Scatter Boost | Same strips and scatter trigger weights as base, but direct freegame criteria quota is `0.108` instead of Base `0.100`. Including the separate `0.001` wincap feature-entry condition, total forced feature-entry allocation is `0.109` instead of Base `0.101`. |
