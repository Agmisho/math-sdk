<script lang="ts">
	...
	const basePanelWidth = $derived(Math.min(canvas.width * (isPortrait ? 0.90 : 0.68), canvas.height * 0.40 * UI_RATIO));
	const basePanelHeight = $derived(basePanelWidth / UI_RATIO);
	const panelWidth = $derived(basePanelWidth * UI_SCALE);
	const panelHeight = $derived(panelWidth / UI_RATIO);
	const panelX = $derived(canvas.width / 2 + basePanelWidth * UI_HORIZONTAL_OFFSET);
	const panelY = $derived(boardLayout.y + boardLayout.frameHeight / 2 + basePanelHeight * UI_VERTICAL_OFFSET);
